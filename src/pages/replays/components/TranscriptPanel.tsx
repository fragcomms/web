...
import { ArrowDownToLine, EllipsisVertical, AlignVerticalSpaceAround } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export const TranscriptPanel = memo(function TranscriptPanel(
  ...
  const centerActiveTranscript = (segmentIndex: number) => {
    const container = transcriptContainerRef.current;
    if (!container) return;

    const activeElement = container.querySelector<HTMLDivElement>(`[data-segment-index="${segmentIndex}"]`);
  ...
  useEffect(() => {
    ...
    if (isSyncEnabled) {
      centerActiveTranscript(firstActiveIndex);
    }
  }, [firstActiveIndex, isSyncEnabled]);

  // detect manual scrolling in the transcript panel to disable auto-sync
  useEffect(() => {
    const container = transcriptContainerRef.current;
    if (!container) return;
      ...
      <div className="mb-2 shrink-0 flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            ...
            aria-label={isSyncEnabled ? "Disable Auto-Sync" : "Enable Auto-Sync"}
            title={isSyncEnabled ? "Disable Auto-Sync" : "Enable Auto-Sync"}
            className={`inline-flex h-8 w-8 items-center justify-center rounded-md border transition ${isSyncEnabled ? "border-cyan-500 bg-cyan-100 text-cyan-700" : "border-slate-300 bg-white text-slate-600 hover:bg-slate-100"} dark:disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900`}
          >
